...
const ips = _(config.agent.manifest.jobs)
  ...
  .flattenDeep()
  .value();
const blueprintCredentials = _.assign(config.agent.manifest.properties.blueprint, {
  ips: ips,
  hostname: ips[0]
});
...
class AgentApi {
  ...
  static getInfo(req, res, next) {
    ...
    pingBlueprint()
      ...
      .catch(() => false)
      .then(operational => {
        const baseFeatures = ['state', 'lifecycle', 'deploymenthooks'];
  ...
  static preUpdate(req, res, next) {
    /* jshint unused:false */
    const context = req.body;
    logger.agent.info(`--- BlueprintAgent/deploymenthooks/preupdate: (${JSON.stringify(context)})`);

    if (!_.isPlainObject(context) || !_.isEmpty(context)) {
      return next(new BadRequest());
    }

    const response = {
      actionResponse: "Blueprint-PreUpdate-Response"
    }
