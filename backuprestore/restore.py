    ...
    try:
        ...
        if encrypted_snapshot_id is not None:
            ...
            # +-> Copy the Encrypted backups contents to the persistent volume
	          if not iaas_client.shell('cp -r {}/blueprint/files/* {}/blueprint/files; exit 0'.format(DIRECTORY_DOWNLOADS, DIRECTORY_PERSISTENT)):
                iaas_client.exit('Could not copy backup content from \'{}/blueprint/files/*\' to \'{}/blueprint/files\'.'.format(DIRECTORY_DOWNLOADS, DIRECTORY_PERSISTENT))
            # +-> Start the service job
            iaas_client.start_service_job()
