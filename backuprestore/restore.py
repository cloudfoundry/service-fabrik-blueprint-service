...
def main():
    ...
    iaas_client = create_iaas_client('restore', configuration, DIRECTORY_PERSISTENT, [DIRECTORY_DOWNLOADS])
    ...
    try:
        ...
        if not iaas_client.download_from_blobstore('{}/{}'.format(backup_guid, metadata_files_name), metadata_files_path):
            iaas_client.exit('Could not download the tarball {} from pseudo-folder.'.format(metadata_files_name, backup_guid))
        encrypted_snapshot_id = str(json.load(open(metadata_files_path))['snapshotId'])

        if encrypted_snapshot_id is None:
            # +-> Create a volume where the downloaded blobs will be stored on
            volume_downloads = iaas_client.create_volume(volume_persistent.size)
            if not volume_downloads:
                iaas_client.exit('Could not create a volume for the downloads.')

            # +-> Attach the download volume to the instance
            attachment_volume_downloads = iaas_client.create_attachment(volume_downloads.id, instance_id)
            ...
            mountpoint_volume_downloads = iaas_client.get_mountpoint(volume_downloads.id)
            ...
            if not iaas_client.delete_directory(DIRECTORY_DOWNLOADS):
                iaas_client.exit('Could not remove the following directory: {}.'.format(DIRECTORY_DOWNLOADS))
            if not iaas_client.create_directory(DIRECTORY_DOWNLOADS):
                iaas_client.exit('Could not create the following directory: {}'.format(DIRECTORY_DOWNLOADS))
            if not iaas_client.format_device(mountpoint_volume_downloads):
                iaas_client.exit('Could not format the following device: {}'.format(mountpoint_volume_downloads))
            ...
            if not iaas_client.download_from_blobstore('{}/{}'.format(backup_guid, tarball_files_name), tarball_files_path):
                iaas_client.exit('Could not download the tarball {} from pseudo-folder.'.format(tarball_files_name, backup_guid))
            ...
            if not iaas_client.unmount_device(mountpoint_volume_downloads):
                iaas_client.exit('Could not unmount the device {}.'.format(mountpoint_volume_downloads))
            if not iaas_client.delete_directory(DIRECTORY_DOWNLOADS):
                iaas_client.exit('Could not remove the following directory: {}.'.format(DIRECTORY_DOWNLOADS))
            ...
            if not iaas_client.delete_volume(volume_downloads.id):
                iaas_client.exit('Could not delete the download volume with id {}.'.format(volume_downloads.id))

        if encrypted_snapshot_id is not None:
            # +-> Create a volume where the downloaded blobs will be stored on
            snapshot_volume = iaas_client.create_volume(volume_persistent.size, encrypted_snapshot_id)
            if not snapshot_volume:
                iaas_client.exit('Could not create a volume for the downloads.')

            # +-> Attach the encrypted backup volume to the instance
            attachment_encrypted_snapshot = iaas_client.create_attachment(snapshot_volume.id, instance_id)
            ...
            mountpoint_encrypted_snapshot = iaas_client.get_mountpoint(snapshot_volume.id, '1')
            if not mountpoint_encrypted_snapshot:
                iaas_client.exit('Could not determine the mountpoint for the download volume (id: {}).'
                                .format(encrypted_snapshot.id))
            ...
            if not iaas_client.delete_directory('{}/blueprint/files/*'.format(DIRECTORY_PERSISTENT)):
                iaas_client.exit('Could not remove the following directory: {}.'.format('{}/blueprint/files'.format(DIRECTORY_PERSISTENT)))
            if not iaas_client.create_directory('{}/blueprint/files'.format(DIRECTORY_PERSISTENT)):
                iaas_client.exit('Could not create the following directory: {}'.format('{}/blueprint/files'.format(DIRECTORY_PERSISTENT)))
            ...
            if not iaas_client.unmount_device(mountpoint_encrypted_snapshot):
                iaas_client.exit('Could not unmount the device {}.'.format(mountpoint_encrypted_snapshot))
            if not iaas_client.delete_directory(DIRECTORY_DOWNLOADS):
                iaas_client.exit('Could not remove the following directory: {}.'.format(DIRECTORY_DOWNLOADS))
            ...
            if not iaas_client.delete_volume(snapshot_volume.id):
                iaas_client.exit('Could not delete the download volume with id {}.'.format(snapshot_volume.id))
